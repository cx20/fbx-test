#!/usr/bin/env python3
"""Check animation curves for Armature node (145081345) in Armature|Anim_0.002 clip."""
...
# Find ACNs targeting Armature
print(f"=== Armature animation in Anim_0.002 (id={ARMATURE_ID}) ===")
for layer_id in layers_for_stack:
    acns = acns_for_layer.get(layer_id, [])
    for acn_id in acns:
        model_id, prop_name = acn_target.get(acn_id, (None, None))
        if model_id != ARMATURE_ID: continue
        print(f"\nACN {acn_id} -> Armature prop={prop_name!r}")
        ...
        for axis, curve_id in sorted(axes_curves.items()):
            ...
            if len(values) == 0:
                print(f"  Axis {axis_name}: NO DATA")
                continue

            vmin, vmax = min(values), max(values)
            print(f"  Axis {axis_name}: {len(times)} keyframes, range=[{vmin:.6f}, {vmax:.6f}]")
            if len(values) <= 5:
                for i, (tt, v) in enumerate(zip(times, values)):
                    print(f"    [{i}] t={tt:.4f}s  val={v:.6f}")
            else:
                print(f"    t=0: {values[0]:.6f}  t_last: {values[-1]:.6f}")
                if vmin == vmax:
                    print(f"    *** CONSTANT (all values = {vmin:.6f}) ***")

# Also check Armature Properties70
armature_node = None
for n in iter_nodes(roots, "Model"):
    if n.props and isinstance(n.props[0], int) and n.props[0] == ARMATURE_ID:
        armature_node = n
        break

if armature_node:
    print(f"\n=== Armature Properties70 ===")
    p70 = find_child(armature_node, "Properties70")
    if p70:
        for p in p70.children:
            if p.name == "P" and p.props:
                print(f"  {p.props[0]!r}: {p.props[1:] if len(p.props)>1 else '?'}")
    else:
        print("  No Properties70")
