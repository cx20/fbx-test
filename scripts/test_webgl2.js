// Smoke-test the WebGL2.0 sample with vCube and AnimatedTriangle.
...
(async () => {
    ...
    try {
        for (const m of MODELS) {
            const url = `http://127.0.0.1:5500/example/webgl2/index.html?model=${encodeURIComponent(m.name)}&animation=0&time=${m.time}${m.extra ?? ''}`;
            console.log(`\n=== ${m.name} ===`);
            console.log(`  ${url}`);
            ...
            try {
                ...
                const out = path.join(__dirname, 'screenshots', `webgl2-${m.file}.png`);
