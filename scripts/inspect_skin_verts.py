#!/usr/bin/env python3
"""Inspect FBX vertex coordinates and mesh node properties."""
import os, sys, io
...
if len(sys.argv) < 2:
    print("Usage: python inspect_skin_verts.py <fbx-file>")
    sys.exit(1)

path = sys.argv[1]
roots = load(path)

# Mesh Model nodes and their properties
print(f"\n=== Mesh Models in {os.path.basename(path)} ===")
for m in iter_nodes(roots, "Model"):
    if m.props and len(m.props) > 2 and m.props[2] == "Mesh":
        ...
        if p70:
            for p in p70.children:
                if p.name == "P" and p.props:
                    k = p.props[0]
                    if k in ("Lcl Translation", "Lcl Rotation", "Lcl Scaling", "PreRotation",
                             "GeometricTranslation", "GeometricRotation", "GeometricScaling"):
                        v = p.props[4:] if len(p.props) > 4 else p.props
                        print(f"    {k}: {v}")

# Vertex ranges
print(f"\n=== Vertex Ranges ===")
for g in iter_nodes(roots, "Geometry"):
    if not g.props: continue
    verts = None
    for c in g.children:
    ...
    zs = [verts[i*3+2] for i in range(n)]
    print(f"  Geo id={g.props[0]} verts={n}")
    print(f"    X: [{min(xs):.3f}, {max(xs):.3f}]")
    print(f"    Y: [{min(ys):.3f}, {max(ys):.3f}]")
    print(f"    Z: [{min(zs):.3f}, {max(zs):.3f}]")
    print(f"    first 4 verts: {[(round(verts[i*3],3), round(verts[i*3+1],3), round(verts[i*3+2],3)) for i in range(min(4, n))]}")

# LimbNode bone transforms
print(f"\n=== LimbNode Transforms ===")
for m in iter_nodes(roots, "Model"):
    if m.props and len(m.props) > 2 and m.props[2] in ("LimbNode", "Limb"):
        print(f"  Bone: id={m.props[0]} name={m.props[1]!r}")
        p70 = find_child(m, "Properties70")
        if p70:
            for p in p70.children:
                if p.name == "P" and p.props:
