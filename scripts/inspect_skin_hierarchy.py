#!/usr/bin/env python3
"""Show OO model-to-model parent connections for skinning analysis."""
import os, sys, io
...
for m in iter_nodes(roots, "Model"):
    if m.props:
        ...
        model_info[mid] = (name, mtype)

print(f"\n=== Model nodes ===")
for mid, (name, mtype) in model_info.items():
    print(f"  {mid}: {name!r} ({mtype})")

# Find Connections
conns = None
for n in roots:
    if n.name == "Connections": conns = n; break

print(f"\n=== Model→Model OO connections (parent links) ===")
if conns:
    for c in conns.children:
        if c.name != "C" or c.props[0] != "OO": continue
        fromId = c.props[1]
        toId = c.props[2]
        if fromId in model_info and toId in model_info:
            fname = model_info[fromId][0]
            tname = model_info[toId][0]
            print(f"  {fromId} ({fname}) -> {toId} ({tname})")
        elif fromId in model_info:
            fname = model_info[fromId][0]
            print(f"  {fromId} ({fname}) -> {toId} (NOT A MODEL - root)")
