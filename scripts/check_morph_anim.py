#!/usr/bin/env python3
"""Check what AnimationCurves and ACN target the BlendShapeChannel."""
import os, sys, io
...
for n in iter_nodes(roots, 'AnimationCurveNode'):
    if n.props: acn_ids[n.props[0]] = n.props[1] if len(n.props)>1 else '?'
for n in iter_nodes(roots, 'AnimationCurve'):
    if n.props: curve_ids[n.props[0]] = n
for d in iter_nodes(roots, 'Deformer'):
    if d.props and len(d.props)>2 and d.props[2] == 'BlendShapeChannel':
        bsc_ids.add(d.props[0])

print(f"=== {os.path.basename(path)} ===")
print(f"  ACNs: {len(acn_ids)}, Curves: {len(curve_ids)}, BlendShapeChannels: {len(bsc_ids)}")

# Walk connections
for n in roots:
    if n.name == "Connections":
        print("\nOP/OO connections involving ACN/Curve/BSC:")
        for c in n.children:
            ...
            # ACN -> BSC
            if fromId in acn_ids and toId in bsc_ids:
                print(f"  [ACN->BSC] {fromId} ({acn_ids[fromId]!r}) -> {toId} prop={prop!r} type={ctype}")
            # Curve -> ACN
            elif fromId in curve_ids and toId in acn_ids:
                curve = curve_ids[fromId]
                times = None
                for cc in curve.children:
                    if cc.name == 'KeyTime' and cc.props: times = cc.props[0]; break
                t_count = len(times) if times else 0
                t_max = times[-1] if times else 0
                print(f"  [Curve->ACN] {fromId} -> {toId} ({acn_ids[toId]!r}) prop={prop!r} type={ctype} keys={t_count} tmax={t_max}")
        break
