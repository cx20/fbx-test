#!/usr/bin/env python3
"""List mesh model names and BlendShapeChannel names for morph GUI design."""
...
for path in sys.argv[1:]:
    ...
    for m in iter_nodes(roots, 'Model'):
        if m.props and len(m.props)>2 and m.props[2]=='Mesh':
            ...
            sep = name.find('\x00')
            print(f"  {(name[:sep] if sep>=0 else name)!r}")
    print("BlendShapeChannels:")
    for d in iter_nodes(roots, 'Deformer'):
        if d.props and len(d.props)>2 and d.props[2]=='BlendShapeChannel':
            name = d.props[1] if len(d.props)>1 else ''
            sep = name.find('\x00')
            print(f"  {(name[:sep] if sep>=0 else name)!r}")
