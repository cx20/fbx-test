#!/usr/bin/env python3
"""Check vCube for LayerElementColor (vertex colors) and material info."""
import os, sys, io
...
path = os.path.join(os.path.dirname(__file__), "..", "assets", "models", "fbx", "vCube.fbx")
roots = load(path)

for geo in iter_nodes(roots, "Geometry"):
    print(f"Geometry: {geo.props[1]!r}")
    print(f"  children: {[c.name for c in geo.children]}")
    color_layer = find_child(geo, "LayerElementColor")
    if color_layer:
        print(f"\n  *** LayerElementColor FOUND ***")
        for c in color_layer.children:
            if c.props:
                p = c.props[0]
                if isinstance(p, list):
                    print(f"    {c.name}: array (len={len(p)}) first 12={p[:12]}")
                else:
                    print(f"    {c.name}: {p}")
            else:
                print(f"    {c.name}: (no props)")
    else:
        print("\n  LayerElementColor NOT found")

# Materials
for n in iter_nodes(roots, "Material"):
    if n.props:
        print(f"\nMaterial: id={n.props[0]} name={n.props[1]!r}")
        p70 = find_child(n, "Properties70")
        if p70:
            for p in p70.children:
                if p.name == "P" and p.props:
                    k = p.props[0]
                    if k in ("DiffuseColor", "Diffuse", "AmbientColor", "EmissiveColor", "SpecularColor", "DiffuseFactor"):
                        print(f"  {k}: {p.props[4:] if len(p.props)>4 else '?'}")
