// Inspect BindPose section in FBX
...
// Find all BindPose nodes
const bindPoses = nodes.filter(n=>n.name==='Pose');
console.log(`Found ${bindPoses.length} Pose nodes`);

for(const pose of bindPoses) {
    const type = pose.props[2];
    console.log(`\n=== Pose: "${pose.props[1]}" type="${type}" ===`);
    const poseNodes = pose.children.filter(c=>c.name==='PoseNode');
    console.log(`  PoseNodes: ${poseNodes.length}`);
    for(const pn of poseNodes) {
        const nodeId = pn.children.find(c=>c.name==='Node')?.props[0];
        const matrixNode = pn.children.find(c=>c.name==='Matrix');
        const matrix = matrixNode?.props[0];
        if(!nodeId) continue;
        const nameStr = `id=${nodeId}`;
        if(Array.isArray(matrix) && matrix.length===16) {
            const translation = [mget(matrix,0,3), mget(matrix,1,3), mget(matrix,2,3)];
            console.log(`  PoseNode ${nameStr}: matrix T=[${translation.map(v=>v.toFixed(4)).join(',')}]`);
            console.log(`    Matrix:\n    ${mfmt(matrix)}`);
        } else {
            console.log(`  PoseNode ${nameStr}: no matrix`);
        }
    }
}
